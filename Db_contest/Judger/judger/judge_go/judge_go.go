...
func JudgeGo(code, username, problemNum string) int {
	...
	if files, err := ioutil.ReadDir(path); err != nil {
		...
		for _, fileInfo := range files {
			...
			if strings.HasSuffix(fileInfo.Name(), ".in") {
				if !RunAndCompare(path+"\\"+fileName+".exe",
...
func RunAndCompare(path, dataPath string, username string) bool {
